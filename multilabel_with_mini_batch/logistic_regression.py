...
if __name__ == "__main__":
    ...
    print('Multilabel classification by logistic regression')
    train_times = time.time()
    model = LogisticRegressionWithLBFGS.train(feature_array, iterations=200, numClasses=3, regParam=1, regType='l2')
    ...
    predictions = idf_testing.map(lambda x: (x[0], float(x[1]), float(model.predict(x[2]))) )
    
    # get F1 score
    metrics = MulticlassMetrics(predictions.map(lambda x: (x[2], x[1])))
