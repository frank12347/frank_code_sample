...
if __name__ == "__main__":
    ...
    sc = SparkContext(appName='SVM multilabels prediction for promotion, general and court cases files with mini batch')
    ...
    def loss_gradient(x, label, para):
        """A function to calculate row loss and gradient"""
        y = 1 if x[0][:2] == label else -1
        product = y * np.dot(para, x[2]) 
        loss = max(0, 1 - product)
        grad = 0 if product > 1 else -y * x[2]
        return loss, grad


    def SVM_model(data, count, batch_size, label):
        """Start gradient descent"""
        # w = [para1, para2, para3, para4, para2000]
        sample_ratio = batch_size / count
        num_iteration = 200
        ...
        regulation = 1 / (regular_parameter * batch_size)


        for i in range(num_iteration):
            featureArray = data.sample(False, fraction=sample_ratio)
            cost, grad = featureArray.map(lambda x: loss_gradient(x, label, para) ).reduce(lambda a,b: [a[0] + b[0], a[1] + b[1]])
            loss = regulation * (1 / 2)  * np.linalg.norm(para) + cost / batch_size
            gradient = regulation * para + grad / batch_size
        ...
        largest_five = np.argsort(para)[-20:]
        print('The top 20 wprds with largest coefficients:')
        top5_words = dictionary.filter(lambda x: x[1] in largest_five).map(lambda x: x[0]).collect()
        print(top5_words)
        return para, loss_array
    
    
    def prediction(x, coef, symbol):
        """A function to predict labels
        output is docID, label, predicted label and theta
        """
        theta = np.dot(x[2], coef)
        label = symbol if theta > 0 else 0
        return x[0], x[1], label, theta
    
    
    def confusion_matrix(x, label):
        z = int(x[1])
        tp = 1 if (x[1] == x[2] and z == label) else 0
        fn = 1 if (x[1] != x[2] and z == label) else 0
        tn = 1 if (x[1] == x[2] and z != label) else 0
        fp = 1 if (x[1] != x[2] and z != label) else 0
        return tp, tn, fp, fn
    ...
    idf_training.cache()
    print('General docs count in traning set', idf_training.map(lambda x: x[1]).filter(lambda x: x == 0).count())
    print('Promotion docs count in traning set', idf_training.map(lambda x: x[1]).filter(lambda x: x == 1).count())
    print('Court case docs count in traning set', idf_training.map(lambda x: x[1]).filter(lambda x: x == 2).count())
    ...
    params, loss = SVM_model(idf_training, training_count, 100, 'AD')
    ...
    params2, loss2 = SVM_model(idf_training, training_count, 100, 'AU')
    ...
    idf_training2.cache()
    training_count2 = idf_training2.count()
    params3, loss3 = SVM_model(idf_training2, training_count2, 100, 'AU')
    ...
    measure(pred_ad, 1)
    print()
    print('Evaluation of predicting court cases by one againset other')
    measure(pred_court, 2)
    print('Evaluation of predicting court cases by hierachical classification')
