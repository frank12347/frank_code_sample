...
if __name__ == "__main__":
    ...
    sc = SparkContext(appName="Taxi linear regression with mini batch")
    ...
    # converted seconds to be minues because errors were too big 
    ...
    loss_array = []
    batch_size = 50
    sample_ratio = batch_size / n_count
    ...
    for i in range(num_iteration):
        ...
        """
        sample = training.sample(False, fraction=sample_ratio)
        rdd2 = sample.map(lambda x: loss_grad(x, theta))
        loss1, grad = rdd2.reduce(lambda a,b: [a[0] + b[0], a[1] + b[1] ] )


        gradient = grad / batch_size
        loss = loss1 / batch_size
    ...
    print('The average sq is', round(ave_error, 2))
