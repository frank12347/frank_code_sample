...
if __name__ == "__main__":
    ...
    sc = SparkContext(appName='MLE mini batch multilabels prediction for promotion, general and court cases files')
    ...
    def loss_and_derivative(x, coeficients, sample_ratio, label):
    ...
    def logistic_regression_paras(data, total_count, batch_size, label):
        """logistic regression with mixmum likelihood"""
        # coef = [intercept, para1, para2, para3, para4]
        sample_ratio = batch_size / total_count
        learning_rate = 0.001
        num_iteration = 300
        ...
        for i in range(num_iteration):
            # sample batches from the rdd
            feature_array = data.sample(False, fraction=sample_ratio)
            rdd = feature_array.map(lambda x: (loss_and_derivative(x, coef, sample_ratio,label)))
        ...
        largest_five = np.argsort(coef)[-20:]
    ...
    params, loss = logistic_regression_paras(idf_training, training_count, 100, 'AD')
    ...
    params2, loss2 = logistic_regression_paras(idf_training, training_count, 100, 'AU')
    ...
    params3, loss3 = logistic_regression_paras(idf_training2, training_count2, 100, 'AU')
